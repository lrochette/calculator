...
var getKeyType = function getKeyType(key) {var
...
var createResultString = function createResultString(key, displayedNum, state) {
  ...
  var keyType = getKeyType(key);var

  firstValue =



  state.firstValue,operator = state.operator,modValue = state.modValue,previousKeyType = state.previousKeyType;
...
var updateCalculatorState = function updateCalculatorState(key, calculator, calculatedValue, displayedNum) {
  var keyType = getKeyType(key);var _calculator$dataset =





  calculator.dataset,firstValue = _calculator$dataset.firstValue,operator = _calculator$dataset.operator,modValue = _calculator$dataset.modValue,previousKeyType = _calculator$dataset.previousKeyType;
...
var updateVisualState = function updateVisualState(key, calculator) {
  ...
  Array.from(key.parentNode.children).forEach(function (k) {return k.classList.remove('is-depressed');});
